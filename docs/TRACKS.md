...
| Workflow / test-infrastructure change | **claimed 2026-08-20** for `scripts/milestone-report.py`, to be written now and **merged only once PR #32 (M1-403) closes** | Three concurrent lanes means every workflow change lands in three open review cycles at once, which is lesson 1 at triple cost. If one is genuinely needed mid-wave, claim the slot here and **say so in the next review request on every open lane** — the reviewer is stateless and reads a format change as a substantive one. Lesson 1: a workflow change is a track and takes a slot. **The 2026-08-20 claim is a deliberate use of the slot's own rule rather than an exception to it:** the script generates the milestone stop-point document `CLAUDE.md` mandates at the end of M0 and M1 (`docs/M0-REVIEW.md` is the format), so what it is *for* is closing a wave — and the wave is not closed. It is claimed now only so a second track cannot take the slot while it is being written. It lands after M1-403 merges, which is the boundary, so no open review cycle ever sees it. If that changes and it must land earlier, say so in M1-403's next review request: a stateless reviewer reads a new script in the diff as a substantive change. Held 2026-08-17 by `test/tmpfs-temp-root` (PR #24) and `chore/review-loop` (PR #25), **both merged and released the same day**. #24 moved pytest's temp root to tmpfs (`tests/conftest.py`): full suite 497.7s → 81.3s, `test_lifecycle.py` 96.5s → 3.2s, because the dev machine's only drive is a 7200rpm platter at 49.6ms/fsync. #25 added `scripts/gate.sh`, `scripts/run-review.sh` and the `fast` hypothesis profile. They landed **mid-wave** by deliberate exception, having been checked against every live branch first — the only conflict was this table. **Say so in the next review request:** the conftest change moves where temp files land, nothing about what is asserted. |
| Next free migration number | *free* — `007` is next | `001`-`006` are immutable on master (`006_non_blank_identifiers.sql` landed with **M1-607**). `004_pipeline_failure_events.sql` landed with **M1-606** and `005_research_run_counters.sql` with **M1-306**, and the pair is worth reading as a case study: both branches were told `004` was free, because a claim lives on its holder's branch and this **migration** column is advisory — `scripts/tracks.py` checks the *dependency* claim and nothing reads this one. M1-606 merged first; M1-306 renumbered to `005` at its daily master merge, which is the designed outcome: `.github/scripts/check-migrations.sh` plus master's up-to-date-branch requirement is the enforcement, and it caught this before either number could be applied twice. Renumbering was safe only because M1-306's `004` had never reached master. |
