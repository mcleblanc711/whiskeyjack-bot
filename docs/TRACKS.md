...
| Workflow / test-infrastructure change | **test/tmpfs-temp-root**, then **chore/review-loop** (stacked on it) | Lesson 1: a workflow change is a track and takes a slot. This one moves pytest's temp root to tmpfs (`tests/conftest.py`): full suite 497.7s → 81.3s, `test_lifecycle.py` 96.5s → 3.2s. It is landing **mid-wave**, deliberately — it touches no file any live branch touches (checked: none of M1-306/310/606 modify `tests/conftest.py` or `pyproject.toml`), and holding it would make every remaining round of this wave pay 8 minutes a gate run. **Say so in the next review request on each live branch:** it changes where temp files land, nothing about what is asserted. |
...
| workflow | test/tmpfs-temp-root | whiskeyjack-tmpfs-temp-root | no | none | 2026-08-17 |
| workflow | chore/review-loop | whiskeyjack-review-loop | no | none | 2026-08-17 |
