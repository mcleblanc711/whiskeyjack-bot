...
*(Swept the **M1-342** row (merged PR #102, round-2 approve, 2026-09-16 — the detection half of
M2-713's Critical state), which is the one drop `finish-item.sh` asked for. Checked before
making it, not assumed: at the moment of the edit `origin` held `master` alone — T-909 had
already merged and its branch was deleted — and `origin/master` was the **only** ref carrying
that row, so after the drop nothing references the deleted branch and `tracks.py live_claims` cannot raise the unresolvable-claim problem that made
`start-item.sh` abort for every item between 2026-09-07 and 2026-09-09. The check is in the
commit that made the edit and refuses if it does not hold. **T-909's own row is left in place**
— it landed after M1-342 and is the next branch's one sweep, not this one's.*

*`T-909` took the **workflow / test-infrastructure slot** and released it at merge. It changed
`tests/unit/test_tournament.py` only — anchoring
`test_a_retired_profile_polled_every_five_minutes_pages_once` to its own tumbling throttle window
instead of wall-clock `utcnow()`, plus the adjacent-window twin. M1-341's review request says so
explicitly, because a stateless reviewer reads a test-file change on a neighbouring lane as
substantive. Lesson 1 in `docs/LESSONS.md` is why the slot exists at all; the cost it prices was
near zero this time, with no other lane open when it started and one when it merged.*
...
| M2-713 | feat/m2-713-reconcile-unrecorded-post | whiskeyjack-m2-713 | no | 016 | 2026-09-15 |
| M1-341 | feat/m1-341-watchdog-resolutions-timer | whiskeyjack-m1-341 | no | none | 2026-09-15 |
...
*(Swept the **M1-351** row (merged PR #122, round-2 approve, deployed 2026-09-23 — the
generation failure path), which `finish-item.sh` left for this branch. Checked first: `origin`
held `master` alone and `whiskeyjack-m1-351` is gone. Earlier sweep note follows.)*
