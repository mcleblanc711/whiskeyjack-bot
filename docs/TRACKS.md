...
Took `M1-502`'s swept table from master rather than re-sweeping Wave 9's rows here, as
the pre-merge note on this branch said to, and then swept the two rows master's version
still carried whose branches `origin` no longer has: `M1-502` merged (PR #49) and
`T-903` merged (PR #50, 2026-08-29), whose `finish-item.sh` run asked the next branch to
drop its row. `M2-708`'s Migration column still reads `none` here and is wrong — it spent
`010` — but that is its own branch's row to correct, and it does; editing it from here
would only manufacture the conflict this table already costs us.)*
