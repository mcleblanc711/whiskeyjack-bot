...
| Claim | Held by | Notes |
| --- | --- | --- |
| Dependency additions (`pyproject.toml` + `uv.lock`) | **`M1-604`** on `feat/m1-604-ledger-exports`, claimed 2026-09-04 | **Spending it on `pyarrow`, and the slot buys a *declaration*, not an install.** M1-604 exports the ledger as JSONL *and* Parquet, and Parquet is a binary columnar container with no stdlib writer. The tempting objection is wheel weight -- `pyarrow` is ~146MB unpacked -- but **the lock disproves it**: `pyarrow 24.0.0` is already resolved into `uv.lock` today, transitively via `forecasting-tools==0.2.92` -> `streamlit 1.59.2` -> `pyarrow>=7.0,<25`, and is already installed in every `.venv` in this project. Declaring it directly adds zero new bytes to any environment. That makes this exactly the `asknews` / `httpx` / `idna` case already commented in `pyproject.toml`: *declared because we import it directly, since an undeclared transitive import breaks whenever the intermediate drops it.* Bound `>=24,<25` rather than pinned exactly -- the bound must stay inside streamlit's `>=7.0,<25` or `uv sync --locked` conflicts, which is the same constraint the `asknews` comment records. `fastparquet` would drag in pandas+numba and `polars` would be a second dataframe engine imported for one `write_parquet` call; neither is already present as a direct import target. Three other lanes are live (`D-1001`, `M2-705`, `M1-407`) and none of them may touch `pyproject.toml`/`uv.lock` while this row stands. Release it when this branch merges. Previously claimed by M1-303 on 2026-07-27 and **released unused** (the Exa adapter used `httpx` instead of adding `exa-py`). **M1-311 claimed it 2026-08-25, spent it, and merged** (PR #40, round-2 approval, 2026-08-25): `publicsuffix2` rejects multi-label suffixes (`co.uk`, `com.au`) that the old dependency-free rule missed. Released now that the branch is on master. |
