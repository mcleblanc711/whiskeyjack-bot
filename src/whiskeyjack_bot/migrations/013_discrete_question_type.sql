-- M1-205: admit `discrete` to the forecast_records question_type vocabulary.
--
-- 008's own comment on this clause anticipated exactly this migration: "Widening this
-- list -- adding `discrete`, say -- costs one more DROP/CREATE of this trigger in a later
-- migration, which is the same cheap escape hatch 004 and 006 used, and not a table
-- rebuild." This is that migration, and it is the fifth rewrite of this trigger (004,
-- 006, 007, 008, 013).
--
-- Found by cross-model review round 3, after two rounds had passed. The item's own
-- planning checked `forecast_records` for a CHECK constraint on `question_type`, found
-- none, and concluded no migration was needed. The vocabulary is enforced by a **trigger**,
-- not a CHECK, so that check looked in the wrong place and every discrete forecast would
-- have converted, validated, and then been refused at persistence with zero rows written --
-- after the model call was billed. Look at `sqlite_master` for triggers, not only at the
-- table DDL.
--
-- The definition below is 008's, byte-identical apart from the single `NOT IN` list, so a
-- diff against 008 shows one changed line and nothing else. Every other clause it carries --
-- the draft-status rule, the JSON shape rules, 007's version chain, 008's relative
-- `raw_output_path` and non-negative `cost_usd` -- is preserved deliberately rather than
-- re-derived: this trigger is immutable once merged, and a rewrite that dropped a clause
-- would be silent.
...
BEGIN
    ...
    WHERE NEW.question_type IS NULL
       ...
       OR NEW.question_type NOT IN ('binary', 'multiple_choice', 'numeric', 'discrete');
